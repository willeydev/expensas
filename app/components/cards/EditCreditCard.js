...
const EditCreditCard = (props) => {
  ...
  function getFlagByValue(code) {
    console.log('flag', code, flags.find(flag => String(flag.value) == String(code))?.label || null);
    return flags.find(flag => String(flag.value) == String(code))?.label || null;
  }
  ...
  useEffect(() => {
    ...
    setDueDay(props.item.dueDay)
    setCloseDay(props.item.closeDay)
    setSelectedFlag(props.item.selectedFlag)
  }, [props.item])
    ...
    <>
        ...
        <View style={[Theme.ModalBody, {minHeight: '83%', maxHeight: '83%',}]}>
            ...
            <InputSelect 
                items={flags}
                type='text'
                label={getFlagByValue(props.item.selectedFlag)}
